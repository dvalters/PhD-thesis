...
class CaesarTimeseriesPlot(object):
    def __init__(self, data_dir, fname, datametric, time_delta=60, time_label='hours',
        ...
        self.start_date = 'Jun 18 2005 00:00'
    ...
    def plot_hydrograph(self, current_timeseries=None,
        ...
        self.ax.set_xlim( [datetime.datetime(2005,6,19,18,0,0), datetime.datetime(2005,6,20,10,0,0)] )
    ...
    def set_labels(self):
        ...
        self.ax.set_xlabel("Time (2005-06-19 to 2005-06-20 UTC)")
    ...
    def plot_external_data(self, filename, cols=(0,1), header=0):
        """
        Plots external data, such as recorded discharge from
        a gauging station
        """
        x, y = np.genfromtxt(filename, skip_header=header, usecols=cols, unpack=True, delimiter=',', missing_values='')
        hours, minutes = convert_timestep(x)
        # Ryedale data is BST so subtract 1 hour
        hours = hours - 1
        minutes = minutes - 60
        times = self.datetimes_from_timestep(self.start_date, minutes)
        line, = self.ax.plot(times, y, ':', linewidth=1.5, color='black', alpha=0.7)
        line.set_label("Gauging station data")        
...
# OOP way
RyeHydroS = CaesarTimeseriesPlot(data_dir, "ryedale*.dat", "q_lisflood",
                                 colormap="ocean")

RyeHydroS.plot_ensemble_hydrograph(draw_inset=False, labellines=False)
#RyeHydroS.shrink_axes_for_legend()
RyeHydroS.plot_external_data(data_dir + external_file_data, cols=(0,1))
RyeHydroS.plot_legend()
