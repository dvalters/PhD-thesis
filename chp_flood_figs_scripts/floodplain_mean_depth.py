...
for file in sorted(glob.glob(bos_file_wildcard)):
    ...
    line, = axarr[0].plot(hours, floodplain_depth, marker=marker.__next__(), linestyle='', alpha=0.7)
    print(line)
    line.set_label(label)

axarr[0].legend(loc="upper left")  
#axarr[0].set_ylim(-0.2, 3.0)
axarr[0].set_ylabel("Mean floodplain\n water depth (m)")
...
for file in sorted(glob.glob(rye_file_wildcard)):
    ...
    hours = convert_timestep(timestep)
    label = make_line_label(file)
    line, = axarr[1].plot(hours, floodplain_depth, marker=marker.__next__(), linestyle='', alpha=0.7)
    print(line)
    line.set_label(label)

axarr[1].legend(loc="upper left") 
axarr[1].set_title("Ryedale") 
axarr[1].set_ylabel("Mean floodplain\n water depth (m)")
